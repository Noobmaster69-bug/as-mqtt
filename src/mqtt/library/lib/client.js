...
MqttClient.prototype.subscribe = function () {
  ...
  if (Array.isArray(obj)) {
    obj.forEach(function (topic) {
      ...
      if (
        !Object.prototype.hasOwnProperty.call(that._resubscribeTopics, topic) ||
    ...
    Object.keys(obj).forEach(function (k) {
      ...
      if (
        !Object.prototype.hasOwnProperty.call(that._resubscribeTopics, k) ||
  ...
  switch (qos) {
    case 2: {
      options.customHandleAcks(topic, message, packet, function (error, code) {
        ...
        if (code) {
          that._sendPacket(
            { cmd: "pubrec", messageId: messageId, reasonCode: code },
            done
          );
        } else {
    ...
    case 1: {
      ...
      options.customHandleAcks(topic, message, packet, function (error, code) {
        ...
        that.handleMessage(packet, function (err) {
          ...
          that._sendPacket(
            { cmd: "puback", messageId: messageId, reasonCode: code },
            done
          );
        });
      });
      ...
      if (this.options.protocolVersion === 5) {
        ...
        for (let topicI = 0; topicI < _resubscribeTopicsKeys.length; topicI++) {
          ...
          this.subscribe(resubscribeTopic, {
            properties:
              resubscribeTopic[_resubscribeTopicsKeys[topicI]].properties,
          });
        }
      } else {
